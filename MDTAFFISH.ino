...
unsigned int ui_Motors_Speed = 1630;//1625                                               // Default run speed
...
unsigned long currentMillis;  //get the current "time" (actually the number of milliseconds since the program started)
    ...
    case 1:    //Robot Run after 3 seconds
      {
        if (bt_3_S_Time_Up)
        {
         ...
         //set motor speeds
          ui_Left_Motor_Speed = constrain(ui_Motors_Speed + ui_Left_Motor_Offset, 1510, 2100);//originally1600
          ui_Right_Motor_Speed = constrain(ui_Motors_Speed + ui_Right_Motor_Offset, 1510, 2100);//originally1600
          ...
          //Ultrasonic
          Ping();
          if((ul_Echo_Time/ 58) < 25)
          {
            period1 = 6000;
                currentMillis = millis();  //get the current "time" (actually the number of milliseconds since the program started)
                previousMillis = 0;
                if (currentMillis - previousMillis >= period1)  //test whether the period has elapsed
                {
                      previousMillis = currentMillis;  //IMPORTANT to save the start time of the current LED state.
                      ui_Left_Motor_Speed = 1700;
                      ui_Right_Motor_Speed = 1100;
                      
                }
                
          }
          if ((ui_Left_Line_Tracker_Data < (ui_Left_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) &&
              (ui_Right_Line_Tracker_Data < (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance) && stopCounter >8))
          {
            ui_Left_Motor_Speed = ci_Left_Motor_Stop;
            ui_Right_Motor_Speed = ci_Left_Motor_Stop;
          }

          //Senses which sensor was last sensed so when it goes off track we can turn it back according to which sensor was last engaged
          int turnLeft = 1;
          int turnRight = 2;
          int inMiddle = 3;
          static int lastSense;
          ...
          //turns the robot according to last sense
          if (((ui_Left_Line_Tracker_Data > (ui_Left_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) &&
               (ui_Middle_Line_Tracker_Data > (ui_Middle_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) &&
               (ui_Right_Line_Tracker_Data > (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))) && (lastSense == 1))
          {
            //turns left
            ui_Left_Motor_Speed = 1490;
            ui_Right_Motor_Speed = 1600;
          }

          if (((ui_Left_Line_Tracker_Data > (ui_Left_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) &&
               (ui_Middle_Line_Tracker_Data > (ui_Middle_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) &&
               (ui_Right_Line_Tracker_Data > (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))) && (lastSense == 2))
          {
            //turns right
            ui_Left_Motor_Speed = 1600;
            ui_Right_Motor_Speed = 1490;
          }
          //To  keep going straight if the last thing detected was a stop sign
          if (((ui_Left_Line_Tracker_Data > (ui_Left_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) &&
               (ui_Middle_Line_Tracker_Data > (ui_Middle_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) &&
               (ui_Right_Line_Tracker_Data > (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))) && lastSense == 4)
            {
            if(lastSense == 4 || lastSense ==3)
            {
              if((ul_Echo_Time / 58) > 25)
              {
              ui_Left_Motor_Speed = 1630; 
                  ui_Right_Motor_Speed = 1630;
                period1 = 50;
                currentMillis = millis();  //get the current "time" (actually the number of milliseconds since the program started)
                previousMillis = 0;
                if (currentMillis - previousMillis >= period1)  //test whether the period has elapsed
          ...
          {
            //Adjust Right
            ui_Left_Motor_Speed = ui_Motors_Speed - 70; //defaultspeed-100
            ui_Right_Motor_Speed = ui_Motors_Speed - 50; //defaultspeed-75
          }
          ...
          {
            //Adjust Left
            ui_Left_Motor_Speed = ui_Motors_Speed - 50; //default speed-50
            ui_Right_Motor_Speed = ui_Motors_Speed - 80; //defaultspeed-100
          }
...
/* if(stopCounter>=1)
      {
          ui_Left_Motor_Speed = 1625;//default speed-100
          ui_Right_Motor_Speed = 1625;//defaultspeed-100
          ...
          (ui_Right_Line_Tracker_Data > (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)))
          {
            ui_Left_Motor_Speed = 1545;//defaultspeed-100
            ui_Right_Motor_Speed = 1650;//defaultspeed-75
          }
          ...
          (ui_Right_Line_Tracker_Data < (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)))
          {
            ui_Left_Motor_Speed = 1650;//default speed-50
            ui_Right_Motor_Speed = 1545;//defaultspeed-100
          }
      }*/
/*Line Tracker*/
...
/*    if((ui_Left_Line_Tracker_Data < (ui_Left_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) &&
     ...
     (ui_Right_Line_Tracker_Data < (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)))
    {
      ...
      /*will only increment once at the stop, by only incrementing if the previous number of LEDs engaged was single or double digit.
      if(lastSense==1||lastSense==2||lastSense==3)
      {
          stopCounter++;
      }*/
