...
SoftwareSerial mySerial(A3, 11); // RX, TX
...
const int ci_Ultrasonic_Ping2 = 11;   //input plug
const int ci_Ultrasonic_Data2 = 10;
...
unsigned int ui_Motors_Speed = 1700;//1625                                               // Default run speed
...
void setup() {
  ...
  pinMode(ci_Ultrasonic_Ping2, OUTPUT);
  pinMode(ci_Ultrasonic_Data2, INPUT);
...
static double detected = 1;
static double detected1 = 1;
static int notDetecting;
static int read = 0;
    ...
    case 0:    //Robot stopped
      {
        ...
        if (mySerial.available())
                ...
                notDetecting=3;
                  ...
                  if( detected1 > 10 && notDetecting==3 ) 
                  {
    ...
    case 1:    //Robot Run after 3 seconds
      {
        if (bt_3_S_Time_Up)
        {
          ...
          Serial.println(l_Right_Motor_Position);
                ...
                if ((mySerial.available()) && ul_Echo_Time/58 > 30 )
                {
          ...
          //Ultrasonic
          /*
          if((ul_Echo_Time/ 58) < 10)
          {
            ui_Left_Motor_Speed = 1720;
            ui_Right_Motor_Speed = 1700;
                
          }
          if((ul_Echo_Time2/ 58) < 45)
          {
...
void Ping()
{
  ...
  Serial.print(", cm: ");
  Serial.print(ul_Echo_Time / 58); //divide time by 58 to get distance in cm
#endif
}
...
void Ping2()
{
  ...
  //use command pulseIn to listen to Ultrasonic_Data pin to record the
  //time that it takes from when the Pin goes HIGH until it goes LOW
  ul_Echo_Time2 = pulseIn(ci_Ultrasonic_Data2, HIGH, 10000);

  // Print Sensor Readings
#ifdef DEBUG_ULTRASONIC
  Serial.print("     Second Sensor ");
  Serial.print(ul_Echo_Time2, DEC);
  Serial.print(", Inches: ");
  Serial.print(ul_Echo_Time2 / 148); //divide time by 148 to get distance in inches
  Serial.print(", cm: ");
  Serial.println(ul_Echo_Time2 / 58); //divide time by 58 to get distance in cm
#endif
}
